...
def test_project_edit_pencil_visibility():
    os.environ['DATABASE_URL'] = 'sqlite:///test_roles.db'
    if os.path.exists('test_roles.db'):
        os.remove('test_roles.db')
    ...
    with app.app_context():
        ...
        project = Project(name='Can edit', description='desc', budget_hours=10, status='ACTIVE')
        ...
        u_pmp = ensure_user('proj_pmp@example.com', pmp, True)
        u_admin = ensure_user('proj_admin@example.com', admin, True)
        u_part = ensure_user('proj_part@example.com', participante, True)
        u_client = ensure_user('proj_client@example.com', cliente, False)

    client = app.test_client()

    def projects_html_for(email):
        resp = client.post('/auth/login', data={'action': 'local', 'email': email, 'password': 'password'}, follow_redirects=True)
        assert resp.status_code == 200
        return client.get('/projects').get_data(as_text=True)

    edit_path = f'/project/{project.id}/edit'

    # PMP
    html = projects_html_for('proj_pmp@example.com')
    assert edit_path in html

    # Admin
    html = projects_html_for('proj_admin@example.com')
    assert edit_path in html

    # Participant
    html = projects_html_for('proj_part@example.com')
    assert edit_path not in html

    # Client
    html = projects_html_for('proj_client@example.com')
    assert edit_path not in html
